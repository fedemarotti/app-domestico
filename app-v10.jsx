...
const PedidoDefinitivo = ({ onVolver, codigoPRE }) => {
  ...
  const imprimirTodo = () => {
    ...
    const bultos = ex.bultos || 1;
    const BULTO_WORDS = ['','UN BULTO','DOS BULTOS','TRES BULTOS','CUATRO BULTOS','CINCO BULTOS','SEIS BULTOS','SIETE BULTOS','OCHO BULTOS','NUEVE BULTOS','DIEZ BULTOS'];
    const bultoPalabra = BULTO_WORDS[bultos] || (bultos + ' BULTOS');

    const fmtDoc = n => '$ ' + Math.round(n).toLocaleString('es-AR');
    const colAll = [...COLORES_JABON,...COLORES_SUAVIZANTE,...COLORES_DETERGENTE];
    const colHex = (name) => { const c = colAll.find(c=>c.label===name); return c ? c.hex : '#e2e8f0'; };
    const colTc  = (name) => { const c = colAll.find(c=>c.label===name); return c ? c.tc  : '#1e293b'; };

    // ── activadores ───────────────────────────────────────────────────────────
    const activadoresItem = (item) => {
      const acts = [];
      const tipo = item.tipo;
      if (tipo === 'jabon') {
        const qty = (item.divisiones||[]).length > 1 ? item.divisiones.length : 1;
        acts.push({ tipo: 'JABÓN', qty });
      } else if (tipo === 'detergente') {
        const qty = (item.divisiones||[]).length > 1 ? item.divisiones.length : 1;
        acts.push({ tipo: 'DETERGENTE', qty });
      } else if (tipo === 'lavandina') {
        acts.push({ tipo: 'LAVANDINA', qty: item.cantidad === 200 ? 2 : 1 });
      } else if (tipo === 'combo') {
        const jabQty = (item.jabon_divs||[]).length > 1 ? item.jabon_divs.length : 1;
        acts.push({ tipo: 'JABÓN', qty: jabQty });
        const detQty = (item.det_divs||[]).length > 1 ? item.det_divs.length : 1;
        acts.push({ tipo: 'DETERGENTE', qty: detQty });
        if (!item.canje_lavandina) {
          acts.push({ tipo: 'LAVANDINA', qty: item.cantidad === 200 ? 2 : 1 });
        }
      }
      return acts;
    ...
    const allActs = {};
    items.forEach(it => activadoresItem(it).forEach(a => { allActs[a.tipo] = (allActs[a.tipo]||0) + a.qty; }));

    // ── spec para cotización ──────────────────────────────────────────────────
    const specCot = (item) => {
      const tipo = item.tipo;
      if (['jabon','suavizante','detergente'].includes(tipo)) {
        const divs = item.divisiones || [];
        if (divs.length > 1) return divs.map((d,i)=>`Bolsa ${i+1}: ${d.color}`).join(' · ');
        return item.color || '';
      }
      if (['lavandina','desodorante'].includes(tipo)) {
        return (item.fragancias_piso||[]).map(f=>f.fragancia+' '+f.cantidad+'lts').join(' · ');
      }
      if (tipo === 'combo') {
        const parts = [];
        if ((item.jabon_divs||[]).length>1) parts.push('Jabón: '+item.jabon_divs.map((d,i)=>`Bolsa ${i+1}: ${d.color}`).join(' · '));
        else if (item.jabon_color) parts.push('Jabón: '+item.jabon_color);
        if ((item.suav_divs||[]).length>1) parts.push('Suavizante: '+item.suav_divs.map((d,i)=>`Bolsa ${i+1}: ${d.color}`).join(' · '));
        else if (item.suav_color) parts.push('Suavizante: '+item.suav_color);
        if ((item.det_divs||[]).length>1) parts.push('Detergente: '+item.det_divs.map((d,i)=>`Bolsa ${i+1}: ${d.color}`).join(' · '));
        else if (item.det_color) parts.push('Detergente: '+item.det_color);
        if (item.canje_lavandina) {
          const fr = (item.fragancias_canje||[]).map(f=>f.fragancia+' '+f.cantidad+'lts').join(' · ');
          parts.push('Canje → Desodorante'+(fr?': '+fr:''));
        } else {
          const fr = (item.fragancias_piso||[]).map(f=>f.fragancia+' '+f.cantidad+'lts').join(' · ');
          if (fr) parts.push('Desodorante: '+fr);
        }
        return parts.join(' | ');
      }
      return '';
    ...
    // ── COTIZACIÓN ────────────────────────────────────────────────────────────
    const envioLinea = esR
      ? { label:'Retira en planta', spec:'', gratis:true }
      : esI
        ? { label:'Envío al interior — VIA CARGO / ANDREANI', spec:'Flete a cargo del cliente', gratis:true }
        : zonaEnvio
          ? { label:`Envío FLEX · ${bultoPalabra}`, spec:`${zonaEnvio.zona} · ${cliente.localidad||ex.localidad||''}`, gratis:false }
          : null;

    const divRow = `<tr><td colspan="2" style="padding:0;"><div style="border-top:1px solid #e2e8f0;margin:0 4px;"></div></td></tr>`;

    const itemRowsHtml = items.map(item => {
      const spec = specCot(item);
      const es4bolsas = ((item.divisiones||[]).length>1 || (item.jabon_divs||[]).length>1) && item.cantidad===200;
      const labelExtra = es4bolsas ? ' — paga por 200 lts (4 bolsas)' : '';
      return `${divRow}
        <tr>
          <td style="padding:8px 4px 2px;font-size:13px;"><span style="color:#0d5c3a;font-weight:900;margin-right:6px;">▸</span><strong>${item.nombre}${labelExtra}</strong></td>
          <td style="padding:8px 4px 2px;font-size:13px;text-align:right;font-weight:700;">${fmtDoc(item.precio)}</td>
        </tr>
        ${spec?`<tr><td colspan="2" style="padding:0 4px 6px 22px;font-size:11px;color:#64748b;">${spec}</td></tr>`:''}`;
    }).join('');

    const envasesRowHtml = env>0?`${divRow}
      <tr>
        <td style="padding:8px 4px 2px;font-size:13px;"><span style="color:#0d5c3a;font-weight:900;margin-right:6px;">▸</span><strong>${env} Envase${env>1?'s':''} (Balde)</strong></td>
        <td style="padding:8px 4px 2px;font-size:13px;text-align:right;font-weight:700;">${fmtDoc(tot.envases)}</td>
      </tr>`:'';

    const extrasRowsHtml = (ex.agregados||[]).map(a=>`${divRow}
      <tr>
        <td style="padding:8px 4px 2px;font-size:13px;"><span style="color:#f97316;font-weight:900;margin-right:6px;">▸</span><strong>${a.nombre}</strong></td>
        <td style="padding:8px 4px 2px;font-size:13px;text-align:right;font-weight:700;">${fmtDoc(parseFloat(a.precio)||0)}</td>
      </tr>`).join('');

    const envioRowHtml = envioLinea?`${divRow}
      <tr>
        <td style="padding:8px 4px 2px;font-size:13px;"><span style="color:#0d5c3a;font-weight:900;margin-right:6px;">▸</span><strong>${envioLinea.label}</strong></td>
        <td style="padding:8px 4px 2px;font-size:13px;text-align:right;font-weight:700;">${envioLinea.gratis?'A coordinar':fmtDoc(tot.envio)}</td>
      </tr>
      ${envioLinea.spec?`<tr><td colspan="2" style="padding:0 4px 6px 22px;font-size:11px;color:#64748b;">${envioLinea.spec}</td></tr>`:''}`:'';

    const htmlCot = `<div style="padding:24px;font-family:Arial,sans-serif;color:#1e293b;max-width:620px;">
      <div style="border-left:5px solid #0d5c3a;padding:10px 0 10px 14px;margin-bottom:20px;">
        <div style="font-size:22px;font-weight:900;color:#0d5c3a;">DOMÉSTICO</div>
        <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:1px;">Química de Limpieza · Cotización</div>
      </div>
      <div style="display:flex;gap:20px;margin-bottom:20px;flex-wrap:wrap;">
        <div style="flex:1;min-width:200px;background:#f8fafc;border-radius:8px;padding:12px 14px;">
          <div style="font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:8px;">Cliente</div>
          <div style="font-size:16px;font-weight:900;margin-bottom:8px;">${cliente.nombre}</div>
          ${cliente.telefono?`<div style="font-size:12px;color:#475569;margin-bottom:3px;"><span style="font-weight:700;">Teléfonos:</span> ${cliente.telefono}</div>`:''}
          ${cliente.email?`<div style="font-size:12px;color:#475569;margin-bottom:3px;"><span style="font-weight:700;">Mail:</span> ${cliente.email}</div>`:''}
          ${cliente.direccion?`<div style="font-size:12px;color:#475569;margin-bottom:3px;"><span style="font-weight:700;">Dirección:</span> ${cliente.direccion}</div>`:''}
          ${cliente.localidad?`<div style="font-size:12px;color:#475569;margin-bottom:3px;"><span style="font-weight:700;">Localidad/Partido:</span> ${cliente.localidad}</div>`:''}
          ${cliente.cp?`<div style="font-size:12px;color:#475569;margin-bottom:3px;"><span style="font-weight:700;">CP:</span> ${cliente.cp}</div>`:''}
          ${cliente.observaciones?`<div style="font-size:12px;color:#92400e;margin-top:6px;padding:6px 8px;background:#fef3c7;border-radius:5px;"><span style="font-weight:700;">Observaciones:</span> ${cliente.observaciones}</div>`:''}
        </div>
        <div style="min-width:150px;background:#f8fafc;border-radius:8px;padding:12px 14px;">
          <div style="font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:8px;">Pedido</div>
          <div style="font-size:13px;font-weight:800;color:#0d5c3a;margin-bottom:4px;">${codigo}</div>
          <div style="font-size:11px;color:#64748b;">${fecha}</div>
        </div>
      </div>
      <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
        <thead><tr style="background:#0d5c3a;">
          <th style="padding:8px;text-align:left;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">Producto / Servicio</th>
          <th style="padding:8px;text-align:right;font-size:11px;color:#fff;text-transform:uppercase;letter-spacing:0.8px;font-weight:700;">Precio</th>
        </tr></thead>
        <tbody>
          ${itemRowsHtml}
          ${envasesRowHtml}
      ...
      <div style="background:#0d5c3a;color:#fff;border-radius:8px;padding:14px 18px;display:flex;justify-content:space-between;align-items:center;">
        <span style="font-size:14px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;">TOTAL A ABONAR</span>
        <span style="font-size:24px;font-weight:900;">${fmtDoc(tot.total)}</span>
      </div>
      <div style="margin-top:16px;font-size:10px;color:#94a3b8;text-align:center;">Precios con 15% OFF · Doméstico Química de Limpieza · Moreno, Bs. As.</div>
    ...
    const htmlProd = `<div style="padding:14px;font-family:Arial;font-size:12px;color:#1e293b;max-width:800px;">
      ...
      ${items.map(item=>{
        ...
        const colInfo = colAll.find(c=>c.label===item.color);
        return `<div style="padding:8px 10px;border-left:4px solid #0d5c3a;background:#f8fafc;margin-bottom:7px;border-radius:4px;">
          <div style="font-weight:800;font-size:13px;">${item.nombre}</div>
          ${hasDivs?divs.map(d=>`<span style="display:inline-flex;align-items:center;gap:4px;background:${colHex(d.color)};color:${colTc(d.color)};border-radius:10px;padding:2px 8px;font-size:11px;font-weight:700;margin:2px;">● 50lts ${d.color}</span>`).join(''):colInfo?`<div style="margin-top:4px;display:inline-flex;align-items:center;gap:5px;background:${colInfo.hex};color:${colInfo.tc};border-radius:10px;padding:2px 8px;font-size:11px;font-weight:700;">● ${colInfo.label}</div>`:''}
          ${item.jabon_divs&&item.jabon_divs.length>1?`<div style="font-size:10px;margin-top:3px;font-weight:700;color:#475569;">Jabón:</div>${item.jabon_divs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 7px;font-size:10px;font-weight:700;margin:1px;display:inline-block;">Bolsa${i+1}: ${d.color}</span>`).join('')}`:''}
          ${item.suav_divs&&item.suav_divs.length>1?`<div style="font-size:10px;margin-top:3px;font-weight:700;color:#475569;">Suavizante:</div>${item.suav_divs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 7px;font-size:10px;font-weight:700;margin:1px;display:inline-block;">Bolsa${i+1}: ${d.color}</span>`).join('')}`:''}
          ${item.det_divs&&item.det_divs.length>1?`<div style="font-size:10px;margin-top:3px;font-weight:700;color:#475569;">Detergente:</div>${item.det_divs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 7px;font-size:10px;font-weight:700;margin:1px;display:inline-block;">Bolsa${i+1}: ${d.color}</span>`).join('')}`:''}
          ${item.fragancias_piso&&item.fragancias_piso.length?`<div style="font-size:10px;margin-top:2px;">Desodorante: ${item.fragancias_piso.map(f=>f.fragancia+' '+f.cantidad+'lts').join(' + ')}</div>`:''}
          ${item.canje_lavandina?`<div style="font-size:11px;font-weight:700;color:#f97316;margin-top:4px;">⚠️ CANJE: Sin lavandina → Desodorante${item.fragancias_canje&&item.fragancias_canje.length?' ('+item.fragancias_canje.map(f=>f.fragancia+' '+f.cantidad+'lts').join(' + ')+')'  :''}</div>`:''}
        </div>`;
      }).join('')}
    ...
    const htmlEtiq = `<div style="width:100mm;min-height:150mm;background:#fff;color:#1e293b;font-family:Arial;padding:8mm;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;">
      <div style="border-bottom:4px solid #0d5c3a;padding-bottom:4mm;margin-bottom:4mm;text-align:center;">
        ...
        <div style="font-size:9px;color:#64748b;">Química de Limpieza · Moreno</div>
      </div>
      <div style="flex:1;">
        <div style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:3px;">Destinatario</div>
      ...
      <div style="background:#0d5c3a;color:#fff;border-radius:6px;padding:10px 12px;text-align:center;margin:6mm 0 4mm;">
        <div style="font-size:22px;font-weight:900;letter-spacing:1px;">${bultoPalabra}</div>
      </div>
      ${esI?`<div style="margin:4px 0;padding:5px 8px;background:#fbbf24;border-radius:5px;text-align:center;font-weight:900;font-size:12px;">📦 VIA CARGO / ANDREANI</div>`:''}
      <div style="border-top:2px solid #e2e8f0;padding-top:3mm;margin-top:2mm;display:flex;justify-content:space-between;align-items:flex-end;">
        <div style="font-size:9px;"><div style="font-weight:700;">${codigo}</div><div style="color:#64748b;">${fecha.split(',')[0]}</div></div>
        <div style="font-size:9px;color:#94a3b8;">Doméstico</div>
      </div>
      <style>@media print{@page{size:100mm 150mm;margin:0;}body{margin:0;}}</style>
    </div>`;

    // ── CHECKLIST (con activadores y extras) ──────────────────────────────────
    const checkItemsHtml = items.map(item => {
      ...
      if (divs.length > 1) {
        return divs.map((d,i) => `
          <div style="display:flex;align-items:flex-start;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
            ...
            <div>
              <div style="font-weight:700;">${item.nombre} — Bolsa ${i+1}/4 (50 lts)</div>
              <span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:10px;padding:1px 7px;font-size:10px;font-weight:700;">● ${d.color}</span>
            </div>
          </div>`).join('');
      }
      if (tipo === 'combo') {
        const jabDivs = item.jabon_divs||[];
        const suavDivs = item.suav_divs||[];
        const detDivs = item.det_divs||[];
        let html = `<div style="padding:7px 0;border-bottom:1px solid #f1f5f9;">
          <div style="display:flex;align-items:center;gap:10px;">
            <div style="width:18px;height:18px;border:2px solid #0d5c3a;border-radius:3px;flex-shrink:0;"></div>
            <div style="font-weight:700;font-size:13px;">${item.nombre}</div>
          </div>`;
        if (jabDivs.length>1) html+=`<div style="margin-left:28px;margin-top:4px;"><span style="font-size:10px;font-weight:700;color:#475569;">JABÓN:</span> `+jabDivs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;margin:1px;">B${i+1}: ${d.color}</span>`).join('')+'</div>';
        else if (item.jabon_color) html+=`<div style="margin-left:28px;margin-top:3px;"><span style="font-size:10px;font-weight:700;color:#475569;">JABÓN:</span> <span style="background:${colHex(item.jabon_color)};color:${colTc(item.jabon_color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;">${item.jabon_color}</span></div>`;
        if (suavDivs.length>1) html+=`<div style="margin-left:28px;margin-top:3px;"><span style="font-size:10px;font-weight:700;color:#475569;">SUAVIZANTE:</span> `+suavDivs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;margin:1px;">B${i+1}: ${d.color}</span>`).join('')+'</div>';
        else if (item.suav_color) html+=`<div style="margin-left:28px;margin-top:3px;"><span style="font-size:10px;font-weight:700;color:#475569;">SUAVIZANTE:</span> <span style="background:${colHex(item.suav_color)};color:${colTc(item.suav_color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;">${item.suav_color}</span></div>`;
        if (detDivs.length>1) html+=`<div style="margin-left:28px;margin-top:3px;"><span style="font-size:10px;font-weight:700;color:#475569;">DETERGENTE:</span> `+detDivs.map((d,i)=>`<span style="background:${colHex(d.color)};color:${colTc(d.color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;margin:1px;">B${i+1}: ${d.color}</span>`).join('')+'</div>';
        else if (item.det_color) html+=`<div style="margin-left:28px;margin-top:3px;"><span style="font-size:10px;font-weight:700;color:#475569;">DETERGENTE:</span> <span style="background:${colHex(item.det_color)};color:${colTc(item.det_color)};border-radius:8px;padding:1px 6px;font-size:10px;font-weight:700;">${item.det_color}</span></div>`;
        if (item.canje_lavandina) {
          const fr = (item.fragancias_canje||[]).length?' ('+(item.fragancias_canje.map(f=>f.fragancia+' '+f.cantidad+'lts').join(' + '))+')':'';
          html+=`<div style="margin-left:28px;margin-top:3px;font-size:10px;font-weight:700;color:#f97316;">⚠️ CANJE → Desodorante${fr}</div>`;
        } else {
          const fr = (item.fragancias_piso||[]).length?(item.fragancias_piso.map(f=>f.fragancia+' '+f.cantidad+'lts').join(' + ')):'';
          if (fr) html+=`<div style="margin-left:28px;margin-top:3px;font-size:10px;color:#475569;"><span style="font-weight:700;">Desodorante:</span> ${fr}</div>`;
        }
        html+='</div>';
        return html;
      }
      const colInfo = colAll.find(c=>c.label===item.color);
      const fragSpec = (item.fragancias_piso||[]).map(f=>f.fragancia+' '+f.cantidad+'lts').join(' + ');
      return `<div style="display:flex;align-items:flex-start;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
        ...
        <div>
          ...
          ${colInfo?`<span style="background:${colInfo.hex};color:${colInfo.tc};border-radius:10px;padding:1px 7px;font-size:10px;font-weight:700;">● ${colInfo.label}</span>`:''}
          ${fragSpec?`<div style="font-size:10px;color:#475569;margin-top:2px;">${fragSpec}</div>`:''}
          ${item.canje_lavandina?`<div style="font-size:10px;font-weight:700;color:#f97316;margin-top:2px;">⚠️ CANJE → Desodorante</div>`:''}
        </div>
      </div>`;
    }).join('');

    const actsEntries = Object.entries(allActs);
    const checkActivadoresHtml = actsEntries.length>0?`
      <div style="font-weight:800;font-size:11px;text-transform:uppercase;margin-top:12px;margin-bottom:6px;color:#0d5c3a;border-top:2px solid #0d5c3a;padding-top:8px;">Activadores</div>
      ${actsEntries.map(([tipo,qty])=>`
        <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
          <div style="width:18px;height:18px;border:2px solid #0d5c3a;border-radius:3px;flex-shrink:0;"></div>
          <div style="font-weight:700;">ACTIVADOR PARA ${tipo} × ${qty}</div>
        </div>`).join('')}`:'';

    const checkExtrasHtml = (ex.agregados||[]).length>0?`
      <div style="font-weight:800;font-size:11px;text-transform:uppercase;margin-top:12px;margin-bottom:6px;color:#f97316;">Extras del vendedor</div>
      ${ex.agregados.map(a=>`
        <div style="display:flex;align-items:flex-start;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
          <div style="width:18px;height:18px;border:2px solid #f97316;border-radius:3px;flex-shrink:0;margin-top:1px;"></div>
          <div>
            <div style="font-weight:700;">${a.nombre}</div>
            ${a.precio?`<div style="font-size:10px;color:#f97316;font-weight:700;">${fmtDoc(parseFloat(a.precio)||0)}</div>`:''}
          </div>
        </div>`).join('')}`:'';

    const checkEnvasesHtml = env>0?`
      <div style="font-weight:800;font-size:11px;text-transform:uppercase;margin-top:12px;margin-bottom:6px;color:#f97316;">Envases</div>
      ${Array.from({length:env}).map((_,i)=>`
        <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
          <div style="width:18px;height:18px;border:2px solid #f97316;border-radius:3px;flex-shrink:0;"></div>
          <span>🪣 Envase vacío (${i+1}/${env})</span>
        </div>`).join('')}`:'';
    ...
    const htmlCheck = `<div style="padding:14px;font-family:Arial;font-size:12px;color:#1e293b;max-width:800px;">
      <div style="border-bottom:3px solid #0d5c3a;padding-bottom:8px;margin-bottom:10px;">
        ...
        <div style="font-size:11px;color:#64748b;">${codigo} · ${fecha} · ${cliente.nombre}</div>
      </div>
      ${checkItemsHtml}
      ${checkActivadoresHtml}
      ${checkEnvasesHtml}
      ${checkExtrasHtml}
      <div style="font-weight:800;font-size:11px;text-transform:uppercase;margin-top:12px;margin-bottom:6px;color:#0d5c3a;">Embalaje y despacho</div>
      <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
        <div style="width:18px;height:18px;border:2px solid #0d5c3a;border-radius:3px;"></div>
        <span>Bultos embalados: <strong>${bultos} (${bultoPalabra})</strong></span>
      </div>
      <div style="display:flex;align-items:center;gap:10px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
        <div style="width:18px;height:18px;border:2px solid #0d5c3a;border-radius:3px;"></div>
      ...
      <div style="display:flex;align-items:center;gap:10px;padding:7px 0;">
        <div style="width:18px;height:18px;border:2px solid #0d5c3a;border-radius:3px;"></div>
        <span>${esR?'Entrega en local':'Envío despachado'}</span>
      </div>
      ${cliente.observaciones?`<div style="margin-top:10px;padding:7px 10px;background:#fef3c7;border-radius:6px;font-size:11px;"><strong>⚠️ Obs:</strong> ${cliente.observaciones}</div>`:''}
      <style>@media print{@page{size:A4;margin:10mm;}}</style>
    </div>`;
